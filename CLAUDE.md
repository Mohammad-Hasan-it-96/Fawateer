...
**Out-of-stock signalling (Plan 011 #8) is a warning, not a block** — separate from the strict-oversell toggle above. `Product.isOutOfStock` is `minStockAlert > 0 && quantity <= 0`: gating on the alert makes "the owner set a low-stock alert" the opt-in for "I track this item's stock", so untracked loose/produce items (which sit at 0 forever) never falsely read as finished. It supersedes `isLowStock` in the product-list chip (red "out of stock" vs amber "low stock"). On **scan** of a finished tracked item, `BillingBloc._onScanBarcode` sets the transient `BillingState.outOfStockScan` **and still adds the line** (overselling stays allowed) — the flag rides through the subsequent `AddProductToCartEvent` (whose `copyWith` preserves it), the POS shows a one-shot red notice, then dispatches `ClearOutOfStockScanEvent`. It fires only from the barcode path, not the tap-to-add picker. `watchAllProducts()` orders by `rowId desc` (Plan 011 #5) so a just-added product is top of the list — no `createdAt` column.
